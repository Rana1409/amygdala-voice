...
#default_text = transcribe_audio(input("Enter a path : "))
...
class UI(QDialog):
    ...
    def __init__(self):
        ...
        self.text_prompt.setPlaceholderText("Selected audio file: None")
        gen_layout.addWidget(self.text_prompt, stretch=1)

        self.browse_audio_button = QPushButton("Browse Audio File")
        gen_layout.addWidget(self.browse_audio_button)
    ...
    def browse_audio_file(self):
        audio_file_path, _ = QFileDialog.getOpenFileName(
            self, "Select an audio file", "", "Audio Files (*.mp3 *.flac *.wav *.m4a)"
        )
        if audio_file_path:
            transcription = transcribe_audio(audio_file_path)
            self.text_prompt.setPlainText(transcription)
